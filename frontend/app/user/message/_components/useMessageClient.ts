...
export function useMessageClient(currentUserId: string) {
  ...
  const setIncomingRingtoneElement = useCallback((node: HTMLAudioElement | null) => {
    incomingRingtoneRef.current = node;
  }, []);

  const setOutgoingRingbackElement = useCallback((node: HTMLAudioElement | null) => {
    outgoingRingbackRef.current = node;
  }, []);
